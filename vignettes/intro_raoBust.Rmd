...
```{r}
glm_test(mpg ~ hp + am, data = mtcars, family = poisson(link = "log"))
```
...
```{r}
summary(glm(mpg ~ hp + am, data = mtcars, family = poisson(link = "log")))
```
...
```{r}
mtcars$cluster <- rep(1:8, each = 4)
gee_test(mpg ~ hp + am, data = mtcars, family = poisson(link = "log"), id = cluster)
```
